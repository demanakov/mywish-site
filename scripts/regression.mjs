...
async function page(options={}) {
  ...
  if (options.javaScriptEnabled !== false) {
    await p.waitForFunction(() => !document.querySelector('.rf-submit')?.disabled);
  }
  return p;
}
...
try {
  for(const width of [320,360,375,768,1024,1440]) {
    console.log(`${engine}: viewport ${width}`);
    const p=await page({viewport:{width,height:900}});
    assert.match(await p.locator('.rf-summary').innerText(), /Дата\s+не выбрана/);
  ...
  const p=await page();
  console.log(`${engine}: order flow`);
  await p.locator('#name').fill('Проверка');
  await p.waitForFunction(() => JSON.parse(sessionStorage.getItem('mywish:draft:v1'))?.order?.name === 'Проверка');
  await p.locator('#phone').fill('+7 (812) 345-67-89');
  await p.waitForFunction(() => JSON.parse(sessionStorage.getItem('mywish:draft:v1'))?.order?.phone === '+7 (812) 345-67-89');
  await p.locator('#messenger').fill('https://t.me/test_user');
  await p.waitForFunction(() => JSON.parse(sessionStorage.getItem('mywish:draft:v1'))?.order?.messenger === 'https://t.me/test_user');
  await p.locator('#wish').fill('Тестовый черновик');
  await p.waitForFunction(() => JSON.parse(sessionStorage.getItem('mywish:draft:v1'))?.order?.wish === 'Тестовый черновик');
  ...
  await p.waitForURL('**/#packages',{waitUntil:'domcontentloaded'});
  ...
  assert.equal(await p.locator('#phone').getAttribute('aria-describedby'),'phone-help');
  ...
  await p.close();
  console.log(`${engine}: no-JS and public pages`);
  ...
  await print.waitForFunction(() => matchMedia('print').matches);
  ...
  for(const route of ['/privacy','/consent','/cookies','/requisites','/menu']) {
    const response=await print.goto(base+route,{waitUntil:'domcontentloaded'});assert.equal(response.status(),200,route);
    assert.ok(await print.locator('h1').innerText());
  }
  assert.equal((await print.goto(base+'/missing-page',{waitUntil:'domcontentloaded'})).status(),404);
  await print.close();
